...
# astropy provides unit system and constants for 
...
# import the GalaxyMass to determine the mass of 
# M31 for each component
...
class M33AnalyticOrbit:
    '''Class that integrates the orbit of M33 based 
    on the analytic form of M31's potential'''
    
    
    def __init__(self, filename):
        '''
        This class uses leapfrog integration to 
        calculate the orbit of M33 around M31,
        using the analytic potentials for each of 
        M31's components.
        ...
        #(the value is 4.498502151575286e-06)
        self.G = const.G.to(u.kpc**3/u.Msun/u.Gyr**2).value
        
        # store the output file name
        self.filename = filename
        
        # get the COM position and velocity of 
        # each galaxy using disk particles
        ...
        # set up initial conditions (M33 pos and 
        ...
        self.Vc = 180  
        # circular speed in km/s (Halo Vcirc  Homework 5)
        
        ### Fudge factor ##  
        #Start with 1.0 and then change to fit the orbit 
        self.fudge =  0.65 # 0.6
    ...
    def DynamicalFriction(self, Msat, Vc, r, v):
        """ Function that returns the deceleration 
        from dynamical friction felt by M33
        owing to its motion through the dark matter halo of M31 
        a =  -0.428 G Msat ln(Lambda)  rmag^2 v/vmag 
        coulomb log Lambda = ln (bmax / bmin) ,  bmin = GMsat/Vc^2
        ...
        Vc: `float`
            Max circular velocity of the host's dark 
            matter halo (km/s)
            
        r: `np.ndarray`
            Position vector of the satellite relative to 
            the host (kpc)
        
        v: `np.ndarray`
            Velocity vector of the satellite relative to
            the host (km/s)
        ...
        DF = -0.428*self.G*Msat*Coulomb/r_mag**2/v_mag*v
    ...
    def LeapFrog(self, dt, r, v):
        '''This method is a leapfrog integrator for 
    ...
    def OrbitIntegration(self, t0, dt, tmax):
        '''This method integrates the orbit of M33
        and stores the position and velocity of M33 
        (in the COM frame of M31) at each timestep in a file. 
        ...
        # add 1 to account for the integer rounding
        
        # set up orbit array, two more rows than t/dt 
        #allow for storing the ICs
        # and account for the rounding of the number 
        #of time intervals to an integer
        orbit = np.zeros([N_steps+1, 7]) 
        # first row will be for the ICs
        ...
        # and store the ICs for M33 in the first 
        #row of the orbit file
        # Initial time, initial position relative 
        #to M31, initial velocity relative to M31
        orbit[0] = t0, *tuple(r), *tuple(v)
        
        
        i = 1 # step counter, starts at 1 since the 
        #0th timestep is the ICs above.
        ...
        while (t < tmax):
            ...
            # update positions and velocities with 
            #the leapfrog function
...
def relative_mag(orbit1, orbit2): 
    """ Function to compute the relative separation 
    and velocity using input orbit files for two galaxies, 
    where the orbit files have `orbit_type` formatting: 
    np.dtype([('t', 'f8'), ('r', 'f8', 3), ('v', 'f8', 3)])
...
def vector_mag(orbit):
    """ Function to compute the magnitude of the
    separation and velocity using an input orbit file, 
    where the orbit files have `orbit_type` formatting:
    np.dtype([('t', 'f8'), ('r', 'f8', 3), ('v', 'f8', 3)])
...
# Compute the orbit
M33.OrbitIntegration(0, 0.001, 10.)  # timestep 0.1 default 
# (accuracy of the calculation impacts errors at larger times)
...
ax.plot(M33Orbit_Hmwk7['t'], M31_M33_R_Hmwk7, 'b', 
        lw=5, label='M31-M33 No DF')

# Plot the simulated separation of M31 and M33
ax.plot(M33SimOrbit['t'], M31_M33_SimR, 'r', lw=5, 
        linestyle=":", label='M31-M33 Simulation')

####### Plot the newly computed orbit with DF
ax.plot(M33Orbit['t'], M31_M33_R, 'g', lw=5, 
        linestyle="--", label='M31-M33 With DF')
...
ax.plot(M33Orbit_Hmwk7['t'], M31_M33_V_Hmwk7, 'b',
        lw=5, label='M31-M33 No DF')

# Plot the simulated velocities of M31 and M33
ax.plot(M33SimOrbit['t'], M31_M33_SimV, 'r', lw=5, 
        label='M31-M33 Simulation')

####### Plot the newly computed orbit velocities with DF
ax.plot(M33Orbit['t'], M31_M33_V, 'g', lw=5, 
        linestyle="--", label='M31-M33 With DF')
...
# We assumed an isothermal sphere, but the halos are represented by Hernquist halos. So the dynamical friction terms
# is too strong. We can introduce a fudge factor of ~ 0.6 to decrease the dynamical friction to give a better fit. 
# 
# Also, there is no MW in this calculation.  
# 
# Tidal forces will be decreasing M33's mass continuously, which is not accounted for. Dynamical friction depends on the satellite mass. 
